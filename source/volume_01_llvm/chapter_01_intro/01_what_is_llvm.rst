...
LLVM 是开源编译器领域最活跃的项目之一。在 2025-01 至 2026-07 的 18 个月间，
``main`` 分支累计 **64,685 次提交**，日均约 120 次提交，来自全球数百名贡献者。

.. list-table:: LLVM 子项目提交热度（2025.01 ~ 2026.07）
   :header-rows: 1
   :widths: 20 15 15
...
MLIR 已超过 Clang，成为 LLVM 社区中**最活跃的子项目**。这反映了
MLIR 在后摩尔时代的编译器基础设施中的战略地位——AI 编译器、硬件设计（CIRCT）、
科学计算等领域的交叉需求让 MLIR 成为 LLVM 生态增长最快的部分。

另一个有趣的指标：18 个月中 **NFC（No Functional Change）** 标签出现了近
2,000 次，说明 LLVM 社区有重度代码重构和审查文化——大量提交是"重构/加注释/改格式"
等非功能性变更。**Revert** 标签也出现了近 2,000 次，多于 Fix 的 446 次，反映
了 LLVM 社区"先合并看结果，不行就 Revert"的 CI 文化。

社区的工作节奏也很有意思：**北京时间 18:00（UTC 10:00）** 是提交高峰时段，
说明核心贡献主要集中在北美/欧洲的工作日上午。周末提交占比约 12%，体现了
开源社区的持续活跃性。
