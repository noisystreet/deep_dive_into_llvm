...
.. admonition:: ThinLTO：Google 如何让 LTO 变得实用
   :class: note

   **全量 LTO** 虽然效果好，但有一个致命的工程问题：**链接太慢了** 。
   想象一下，Chromium 有数万个编译单元，如果全部合并成一个巨大的 IR
   模块来做优化——链接时间可能从几秒变成**几十分钟甚至几小时** 。
   这个代价大多数项目无法接受。

   2014 年，Teresa Johnson 和 Xinliang Li（Google）提出了 **ThinLTO** 。
   它的思路很巧妙：不做全量 LTO 那种"全部合并到一处"的激进优化，
   而是让每个编译单元各自优化，但在优化时**通过索引文件了解**其他单元
   提供了什么函数。

   ThinLTO 的"薄"体现在两个层面：

   1. **精简的索引** ：每个编译单元只输出很小的索引文件（几 KB），
      包含导入了哪些函数签名。链接时把所有索引合并，每个编译单元
      就能"看到"全貌，但不需要加载全部 IR
   2. **并行导入** ：所有编译单元同时导入需要的函数，利用多核并行

   效果：
   - 构建时间增长仅 **5-20%** （全量 LTO 是 100-300%）
   - 性能提升达到全量 LTO 的 **95% 左右**
   - 内存开销大幅降低

   Chromium 在 2016 年迁移到 ThinLTO 后，构建时间只增加了 10%，
   但生成了更快的二进制文件。今天，Android 系统的所有原生代码都
   使用 ThinLTO 构建。

   Google 在推广 ThinLTO 时贡献了 LLVM 中 LTO 基础设施的大部分代码，
   包括：模块索引（Module Index）、摘要文件（Summary File）、
   跨模块导入（Function Import）等。这些基础设施后来也被 Apple
   的 Xcode 和其他 LLVM 用户广泛采用。
