...
.. admonition:: TableGen 的由来：一个"副业"项目
   :class: note

   TableGen 最初是 llvm-tblgen 的一个"副产品"。2003 年 LLVM 刚发布时，
   每个后端都需要手写大量重复代码：指令编码、寄存器描述、汇编解析器……
   这些代码的模式高度相似——都是"先定义，然后生成 C++ 代码"。

   为了替代手写，LLVM 团队写了一个代码生成器——这就是 TableGen。
   它的名字很直白：\ "Table"（表格）+ "Gen"（生成器）——把描述性的
   "表格"转换为 C++ 代码。

   但 TableGen 的设计引发过一个有趣的技术争论： **为什么不用已有的工具？**
   
   - 有人提议用 Lex/Yacc 做指令描述——但它们是为"解析文本"设计的，
     不是为"描述数据结构"设计的
